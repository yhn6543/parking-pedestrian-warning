...
RESULT_VIDEO_PATH = RESULT_DIR / "step07_alert_logging_video.mp4"
PREVIEW_FRAME_PATH = DEBUG_DIR / "step07_alert_preview_frame.jpg"
SUMMARY_JSON_PATH = DEBUG_DIR / "step07_alert_summary.json"
LOG_CSV_PATH = LOG_DIR / "step07_risk_log.csv"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Step 07 alert capture and CSV logging test")
...
def print_header(args: argparse.Namespace, mode: str) -> None:
    print("[STEP] Step 07 - Alert Capture and CSV Logging")
    print(f"[MODE] {mode}")
    print()
    print("[INPUT]")
...
def print_success(summary: dict) -> None:
    ...
    print(f"* csv log: {display_path(LOG_CSV_PATH)}")
...
def print_failure(error: Exception) -> None:
    print("[STATUS] FAILED")
    print(f"[ERROR] {error}")
    print("[SUGGESTION] 영상 경로, danger_zone.json, YOLO 모델 파일, CSV/캡처 저장 경로를 확인하세요.")


def draw_alert_logged_label(frame) -> None:
    cv2.putText(
        frame,
        "ALERT LOGGED",
        (16, frame.shape[0] - 24),
        ...
        try:
            while processed_frames < args.max_frames:
                ...
                if log_result["logged"]:
                    ...
                    draw_alert_logged_label(visualized)
