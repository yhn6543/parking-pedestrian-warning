...
> 대표 화면은 재배포 가능한 캡처를 확인한 뒤 추가할 예정입니다. 공개 패키지에는 데이터, 영상, 모델 weight가 포함되지 않습니다.
...
두 결과 모두 동일한 자체 데이터 구성으로 학습한 **Fine-tuned YOLO26n**의 validation 결과입니다. 기본 pretrained 모델과의 비교나 최종 test set 결과가 아닙니다.

| 모델 | Precision | Recall | mAP50 | mAP50-95 |
|---|---:|---:|---:|---:|
| Fine-tuned YOLO26n, imgsz 640 | 0.844 | 0.639 | 0.743 | 0.483 |
| Fine-tuned YOLO26n, imgsz 768 | 0.852 | 0.666 | 0.767 | 0.510 |

프로젝트에서는 위험 보행자를 놓치지 않는 것이 중요하므로 Recall을 우선 확인했습니다. Ultralytics validation에 표시되는 추론 시간은 전체 Streamlit 파이프라인의 실제 FPS와 같지 않습니다.
...
공개 준비 시점의 원본 프로젝트 전체 테스트 결과는 `334 passed`를 목표 기준으로 사용합니다. 테스트는 실제 카메라, 장시간 학습, test set 평가를 실행하지 않습니다.
...
데이터, weight, 실행 결과, 검수 DB와 개인 환경 파일은 공개 패키지에서 제외합니다.
